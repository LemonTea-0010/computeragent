...
const LOCAL_MOCK_DATA = {
  cards: [
    { label: '今日采集任务数', value: 156 },
    { label: '实时运行中任务', value: 34 },
    { label: '总采集数据量(万)', value: 2341 },
    { label: '数据质量平均值', value: 94 },
  ]
}
...
const cards = ref<Array<{label:string;value:number}>>(LOCAL_MOCK_DATA.cards)
const taskVideoFeeds = [
  { id: 1, name: '微博数据流', src: '/9c9533a8caf670f257b86f4cb15af992.mp4' },
  { id: 2, name: '新闻数据流', src: '/a311c1d4ab7627b22b47a5570d196801.mp4' },
  { id: 3, name: '短视频数据流', src: '/c3b7b482c39c84d8c0c230f1e9ff4ee5.mp4' },
]
...
const fetchData = async () => {
  ...
  try {
    ...
    const resp = await Promise.race([getCollectionOverview(), timeoutPromise]) as any
    const axiosData = resp && resp.data !== undefined ? resp.data : resp
    ...
    cards.value = LOCAL_MOCK_DATA.cards
    error.value = '获取采集概览数据失败，已使用本地数据'
  } finally {
    loading.value = false
  }
}
...
const crawlerLogs = ref<CrawlerLog[]>([
  { id:1, agent:'航司监控Agent', platform:'新闻', keyword:'燃油费上调',   action:'采集中', count:142, time:'10:32:15', status:'running' },
...
const previewData = ref<PreviewItem[]>([
  { id:1,  platform:'新闻', content:'【调价公告】国航、厦航、中联航宣布自4月5日起上调燃油附加费。800公里内由10元涨至60元（5倍），800公里以上由20元涨至120元（6倍）。中东地缘冲突导致航油成本飙升。', author:'民航资源网', time:'10:32', sentiment:'负面', heat:12560 },
  { id:2,  platform:'微博', content:'航司燃油费涨了这么多？800公里以上竟然涨了6倍，清明五一出行的成本又要增加了，心好累。', author:'旅游达人小王', time:'10:30', sentiment:'负面', heat:9821 },
  { id:3,  platform:'新闻', content:'国家网信办发布AI模型备案最新进展，312家企业完成备案，监管体系逐步完善。', author:'新华网官方', time:'10:25', sentiment:'正面', heat:8934 },
...
const globeCoordMap: Record<string, [number, number]> = {
  中国: [116.4074, 39.9042],
  美国: [-74.006, 40.7128],
  英国: [-0.1276, 51.5072],
  德国: [13.405, 52.52],
...
const globeCategoryMap: Record<string, GlobeNode['category']> = {
  中国: 'official',
  美国: 'media',
  英国: 'public',
  德国: 'allied',
...
const hourlyGlobeHeatMap: Record<number, Record<string, number>> = {
  8: { 中国: 1180, 美国: 820, 英国: 460, 德国: 430, 日本: 560, 新加坡: 390, 澳大利亚: 330 },
  10:{ 中国: 2341, 美国: 1780, 英国: 920, 德国: 870, 日本: 1120, 新加坡: 760, 澳大利亚: 650 },
  12:{ 中国: 2789, 美国: 2030, 英国: 1020, 德国: 960, 日本: 1250, 新加坡: 840, 澳大利亚: 740 },
  14:{ 中国: 2456, 美国: 1860, 英国: 980, 德国: 900, 日本: 1180, 新加坡: 810, 澳大利亚: 700 },
  20:{ 中国: 1560, 美国: 1210, 英国: 680, 德国: 620, 日本: 760, 新加坡: 590, 澳大利亚: 510 },
}

const nearestHour = computed(() => {
  const keys = Object.keys(hourlyGlobeHeatMap).map(Number).sort((a, b) => a - b)
  return keys.reduce((prev, cur) => Math.abs(cur - selectedHour.value) < Math.abs(prev - selectedHour.value) ? cur : prev)
})
...
const filteredPreview = computed(() => {
  return previewData.value.filter(item => {
    ...
    const matchPlatform = platformFilter.value === '全部' || item.platform === platformFilter.value
    return matchKeyword && matchPlatform
  })
})

const globeNodes = computed<GlobeNode[]>(() => {
  const currentHeat = hourlyGlobeHeatMap[nearestHour.value] ?? hourlyGlobeHeatMap[10]
  return Object.entries(currentHeat).map(([name, value]) => ({
    name,
    value,
    coord: globeCoordMap[name],
    category: globeCategoryMap[name] ?? 'public',
  }))
})

const globeLinks = computed<GlobeLink[]>(() => {
  const currentHeat = hourlyGlobeHeatMap[nearestHour.value] ?? hourlyGlobeHeatMap[10]
  return Object.keys(currentHeat)
    .filter((name) => name !== '中国')
    .map((name) => ({
      source: '中国',
      target: name,
      value: Math.max(180, Math.floor((currentHeat[name] + currentHeat['中国']) / 5)),
      category: globeCategoryMap[name] ?? 'media',
    }))
})
...
onMounted(async () => {
  loading.value = true
  try {
    const resp = await getCollectionOverview()
    const axiosData = resp?.data ?? resp
    const payload = (axiosData && (axiosData.data ?? axiosData)) as { cards?: Array<{label:string;value:number}>; recentTasks?: Task[] }
    cards.value = payload?.cards ?? []
    tasks.value = payload?.recentTasks ?? []
  } catch (e) {
    console.error('获取采集总览数据失败', e)
    cards.value = []
    tasks.value = []
  } finally {
    loading.value = false
    ...
    <div class="main-grid">
      ...
      <div class="left-col">
        ...
        <div class="tech-card task-panel">
          ...
          <div class="task-video-grid">
            <div v-for="feed in taskVideoFeeds" :key="feed.id" class="task-video-card">
              <video
                class="task-video"
                :src="feed.src"
                autoplay
      ...
      <div class="right-col-wrap">
        ...
        <div class="tech-card spatiotemporal-panel">
          ...
          <div class="st-layout">
            <!-- 地图 -->
            <div class="st-map">
              <div class="st-map-title">🌐 全球空间分布 — {{ selectedHour }}:00 数据采集链路</div>
              <CyberGlobeMap :nodes="globeNodes" :links="globeLinks" :data="currentMapData" />
            </div>
            <!-- 时间轴 -->
            <div class="st-time">
...
.scan-line {
  ...
  width:100%;
  height:2px;
  background:linear-gradient(90deg, transparent 0%, rgba(0,240,255,0.05) 10%, rgba(0,240,255,0.95) 50%, rgba(0,240,255,0.05) 90%, transparent 100%);
  box-shadow:0 0 8px rgba(0,240,255,0.7);
  animation:scan-vertical 2.8s linear infinite;
}
.scan-glow {
  position:absolute;
  inset:0;
  background:linear-gradient(180deg, rgba(0,240,255,0.06), transparent 38%, transparent 62%, rgba(0,240,255,0.08));
  pointer-events:none;
}
.feed-name {
  position:absolute;
...
:deep(.el-table__header-wrapper th) {
  background: rgba(7, 16, 36, 0.98) !important;
